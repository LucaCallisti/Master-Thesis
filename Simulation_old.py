...
def simulation():
    ...
    save_funz = Save_exp.SaveExp('/home/callisti/Thesis/Master-Thesis/Result')
    ...
    for i_run in FinalDict.keys():
        ...
        save_auxiliary_fun(Result_1_order, Grad_1_order, Loss_1_order, i_run, 1, save_funz)
        ...
        save_auxiliary_fun(Result_2_order, Grad_2_order, Loss_2_order, i_run, 2, save_funz)
    ...
    print(f'Elapsed time for simulating SDE: {int(hours_tot):02}:{int(minutes_tot):02}:{seconds_tot:05.2f}')
    save_funz.save_result_continuous(Result_1_order, order = 1)
    save_funz.save_result_continuous(Result_2_order, order = 2)
    save_funz.save_comparison_1_and_2_order()
    save_funz.save_loss_sde(Loss_1_order, order = 1)
    save_funz.save_loss_sde(Loss_2_order, order = 2)
    save_funz.save_grad_sde(Grad_1_order, order = 1)
    save_funz.save_grad_sde(Grad_2_order, order = 2)
    save_funz.save_dict()

def save_auxiliary_fun(Result, Grad, Loss, i_run, order, save_funz):
    save_funz.partial_result(Result, 'Result_'+str(order)+'_order', Bool = True)
    save_funz.partial_result(Grad, 'Grad_'+str(order)+'_order', Bool = True)
    save_funz.partial_result(Loss, 'Loss_'+str(order)+'_order', Bool = True)

    if i_run & 3 == 0 or i_run == 1:
        save_funz.save_result_continuous(Result, order = order)
        save_funz.save_loss_sde(Loss, order = order)
        if order == 2:
            save_funz.save_comparison_1_and_2_order()
