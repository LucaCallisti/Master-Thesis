...
def Test_for_f_and_Sigma():
    ...
    f = function_f_and_Sigma(model, dataset.x_train[:10000], dataset.y_train[:10000])
    start = time.time()
    aux = f.apply_f()
...
def Test_SDE():
    ...
    f = function_f_and_Sigma(model, params, names, dataset.x_train[:256], dataset.y_train[:256])
    ...
    result = torchsde.sdeint(sde, x0.unsqueeze(0).to('cuda'), t, method = 'euler', dt =1e-3)
