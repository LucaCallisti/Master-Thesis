...
class function_f_and_Sigma():
    def __init__(self, model, params, names, x_train, y_train, eps_sqrt = 1e-5, Verbose = True):
    ...
    def Calculate_hessian_gradient(self, new_parameters):
        ...
        for  i in range(self.x_train.shape[0]):
            ...
            print(f'    Elapsed time for Gradient: {time.time() - start:.2f} s')
            self.Gradient_all_data[i] = torch.cat([v.flatten() for v in J])
        print(f'Elapsed time for Hessian and gradient: {time.time() - start1:.2f} s')
